...
def account():
    ...
    ##lavel
    acc=Label(root,text="Deposite Balance",font=("lato",40,"bold"),foreground='#5449F1',background='white').place(relx=.5,rely=.05,anchor=CENTER)
    ...
    withdraw=Label(root,text="Deposite  Balance: ",background='#5449F1',foreground='white',font=('times romans',20,'bold')).place(relx=.03,rely=.6)
    ...
    def confirm():
        ...
        if(id_e.get()=="" or newbalance_e.get()=="") :
             MessageBox.showinfo("Deposite process","Search and Submit first ")
             return 0
        else:
            ...
            sql= (
                "INSERT INTO deposite(id,current_balance,deposite_amount,new_balance,date)"
                "VALUES (%s, %s, %s,%s,%s)"
                )
            ...
            top.title(" Deposite Balance!")
            top.config(bg='#5449F1')
            
            text1="Successfully "+str(wb)+"TK Deposite to account ID: "+str(Id)+"Tk\n  Current balance is "+str(nbal)+"TK\n    Thank you !"
